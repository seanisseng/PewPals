...
In our busy lives, it's easy for conversations to stay surface-level. The Pew Pal Bot helps guide discussions toward deeper, more spiritually enriching topics by suggesting relevant questions and collecting prayer requests for groups to pray through together.
...
- **Prayer Request Capture**: In a group chat, users can submit prayer requests with `/pray <request>` or a message starting with `Prayer request:` and review them later with `/prayerlist`.  
...
| `/prayerlist`       | In a private chat, forward a message from a group (or pass the group chat id or group name) to retrieve that group's prayer requests if you are a member. |
...
5. Use `/prayerlist` to print the collated requests for the chat.
6. To fetch a group's requests from a private chat with the bot, forward a message from that group to the bot or run `/prayerlist <group name|chat_id>`.
